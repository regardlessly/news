...
def main():
    ...
    # 5. Filter for senior-relevant articles only
    logger.info("Filtering for senior-relevant articles...")
    section_labels = {
        "singapore": "Singapore", "asia": "Asia", "world": "World",
        "business": "Business", "sport": "Sport",
    }

    # Group scraped articles by section
    by_section: dict = {}
    for art in raw_articles:
        s = (art.get("section") or "other").lower()
        by_section.setdefault(s, []).append(art)

    senior_articles: list = []
    for section, arts in by_section.items():
        label = section_labels.get(section, section.title())
        logger.info(f"  Selecting senior-relevant articles for '{label}' ({len(arts)} scraped)...")
        selected = summariser.select_senior_articles(label, arts, top_n=20)
        senior_articles.extend(selected)
        logger.info(f"  → {len(selected)} senior-relevant articles kept")

    filtered_out = len(raw_articles) - len(senior_articles)
    logger.info(f"Senior filter: {len(senior_articles)} kept, {filtered_out} filtered out")

    # 6. Insert only senior-relevant articles into DB
    logger.info("Inserting senior-relevant articles into database...")
    inserted: list = []
    skipped = 0

    for art in senior_articles:
        try:
    ...
    # 8. Summarise articles that are missing a summary
    ...
    # 9. Embed articles that have summaries but no embedding yet (PostgreSQL only)
    if database.USE_PG:
    ...
    logger.info(
        ...
        f"Scraped: {len(raw_articles)} | "
        f"Senior-relevant: {len(senior_articles)} | "
